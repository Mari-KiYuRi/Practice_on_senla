...
function repository( id, name, visibility, description, language, stars, forks ){
...
function displayRepoDetails(repo) {
    ...
    details.innerHTML = `
        <h2>${repo.name}</h2>
        <p>Discription: ${repo.description || 'none'}</p>
...
reposContainer.addEventListener('click', function(e) {
    const card = e.target.closest('.border-repository');
    if (!card) return;
